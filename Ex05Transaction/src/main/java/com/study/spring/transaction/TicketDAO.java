...
{
	/*
	멤버변수선언 : Spring-JDBC를 사용하기 위한 멤버변수와 setter를 
		정의한다. 또한 트랜잭션 처리를 위한 메니져 클래스의 멤버변수와
		setter를 정의한다. 
	 */
	...
	멤버변수를 초기화한다. 멤버변수를 지정시 <property> 엘리먼트를 
	사용한다. 
	 */
	...
	public void buyTicket(final TicketDTO dto)
	{
		...
		{
			/*
			티켓 구매 금액에 대한 DB처리를 한다. insert계열의 쿼리문
			이므로 update() 메서드를 사용한다. 
			PreparedStatementCreator를 익명클래스로 선언하여 쿼리문
			의 인파라미터를 설정한 후 실행한다. 
			 */
			...
			//티켓매수가 5장 이하라면 정상처리되어 모든 작업이 실제
			//테이블에 commit 된다.
			System.out.println("카드결재와 티켓구매 모두 정상처리 되었습니다.");
			transactionManager.commit(status);
		} catch (Exception e)
		{
			//매수가 초과하면 쿼리에러가 발생하므로 모든 작업은 
			//rollback 처리된다. 

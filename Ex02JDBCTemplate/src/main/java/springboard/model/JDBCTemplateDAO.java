...
※만약 위 메서드에서 파라미터가 필요한 경우에는 Object형 배열을 매개변수로
추가할 수 있다. 
 */
...
//		sql += " ORDER BY idx DESC";
	...
	{//폼값이 저장된 DTO(커맨드객체)을 매개변수로 받아온다.
		/*
		insert와 같이 행의 변화가 생기는 쿼리문을 실행할때는 update()
		메서드를 사용한다. PreparedStatementCreator()를 익명클래스로
		정의하여 메서드를 오버라이딩 한 후 인파라미터가 있는 쿼리문에
		값을 설정한 후 실행한다. 
		 */
		int result = template.update(new PreparedStatementCreator()
		{
			...
			public PreparedStatement createPreparedStatement(Connection con) throws SQLException
			{
				...
				/*
				답변형 게시판에서는 원본글인 경우 idx와 bgroup은 
				같은 값을 입력받게된다. 이때 nextval을 한문장에서
				두번 사용하게 되는데, 한문장에서 두번 나오더라도
				항상 하나의 시퀀스만 반환하게된다. 
				 */
	...
	public void updateHit(String idx)
	{
		...
		에는 생략할 수 있다. 여기서는 필요하지 않아 void로 선언되어
		있다. 
		 */
	}
	...
	public SpringBoardDTO view(String idx)
	{
		...
		/*
		queryForObject()메서드는 쿼리문을 실행한 후 반드시 하나의
		결과를 반환해야 한다. 그렇지 않으면 예외가 발생하므로 반드시
		예외처리를 해주는것이 좋다. 
		 */
		try
		{
			/*
			인파라미터가 있는 쿼리문을 실행할 경우 세번째 인수로 
			Object형 배열을 선언한 후 값을 할당하면 순서대로 
			인파라미터에 추가된다. 
			 */
			dto = template.queryForObject(sql, 
					...
					new Object[] {idx});
			/*
			BeanPropertyRowMapper객체는 쿼리의 실행결과를 DTO에
			저장해주는 역할을 한다. 이때 테이블의 컬럼명과 DTO의 
			멤버변수명이 일치해야한다.
			따라서 테이블의 컬럼명, 작성폼의 name속성값, DTO의 
			멤버변수를 모두 하나의 이름으로 일치시켜야 한다.  
			 */
		} catch (Exception e)
		{
	...
	public int password(String idx, String pass)
	{
		...
		try
		{
			...
			예외가 발생하게되므로 반드시 예외처리를 해야한다. 
			 */
			SpringBoardDTO dto = template.queryForObject(sql, 
					new BeanPropertyRowMapper<SpringBoardDTO>(SpringBoardDTO.class),
					new Object[] {pass, idx});
			/*
			인파라미터가 2개이므로 이를 대체할 값도 Object형 배열에
			추가해주면된다. 
			 */
		...
		/*
		일련번호는 시퀀스를 사용하므로 반드시 1이상의 값을 가지게된다. 
		따라서 초기값인 0을 반환한다면 패스워드 검증에 실패한 것으로 
		판단할 수 있다. 
		 */
		return resultIdx;
	}
	...
	public void reply(final SpringBoardDTO dto)
	{
		...
		사용하면되고, bgroup은 원본글과 동일한 값을 입력해야한다. 
		...
		template.update(sql, new PreparedStatementSetter()
		{
			...
			public void setValues(PreparedStatement ps) throws SQLException
			{
				...
				/*
				답변글은 원본글 아래쪽에 노출되어야 하고, 또한 들여쓰기도
				되어야한다. 따라서 원본글에 +1 해준후 입력해야한다.
				 */
	...
	원본글에 답변을 2번 이상 작성하는 경우 step에 입력되는 값이 동일해
	지는 현상을 방지하기 위한 목적으로 업데이트 하는것이다. 
	 */
